...
class AnalyzeData:
    ...
    def stationarytest(self, x, name):
        ...
        plt.plot(df, label=("Original"+str(name)) )
...
b=b.diff(20)
b.diff(1)
#analyze.stationarytest(b, "RawDMI")
...
c=pd.DataFrame(c)
c=c.diff(20)
#analyze.stationarytest(c, "RawNino1+2")


d=reader.ElNino_list[1]
d=separateByMonth(reader.sortedKeys_Nino[1], d, [8, 9, 10, 11, 12])
d=[i+5 for i in d]
d=pd.DataFrame(np.log(d))
#analyze.stationarytest(d, "RawNino3")
